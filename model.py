...
observed_matches = observed_matches[observed_matches['wteam'].isin(mm_teams) | observed_matches['lteam'].isin(mm_teams)].reset_index()
teams = list(set(np.hstack((observed_matches['lteam'].unique() , observed_matches['wteam'].unique()))))
...
def played_winner(predicted_score=predicted_score):
    ...
    for _,m in bracket.iterrows():
        ...
        if predicted_score[match_ind][0]>predicted_score[match_ind][1]:
            ...
            played[match_ind] = 0.
    played[-1] = team_to_ind[winner['R6CH']]
    return played
...
model.sample(iter=100000, burn=5000, thin=10)
